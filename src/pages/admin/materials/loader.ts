...
import { ensureListMaterials } from "@/services/material/queries/useGetListMaterials";
import type { LoaderFunctionArgs } from "react-router";
import { redirect } from "react-router";

// Loader for materials list page - fetches all materials with pagination
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  if (pageParam !== null) {
    ...
    if (Number.isNaN(parsedPage) || parsedPage < 1) {
      ...
      const cleanUrl = new URL("/admin/materials", request.url);
